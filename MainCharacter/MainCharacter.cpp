...
void AMainCharacter::Tick(float DeltaTime)
{
	...
	//DrawDebugCapsule(GetWorld(), TriggerCapsuleComponent->GetRelativeLocation(), TriggerCapsuleComponent->GetScaledCapsuleHalfHeight(),
	//	TriggerCapsuleComponent->GetScaledCapsuleRadius(), FQuat(FRotator(0.0f, 0.0f, 0.0f)), FColor::Blue, false, 0.01f, 0, 1.0f);
	...
	if (bIsControllable && Controller && !bIsOnLedge)
	{
		...
		if (AxisValue != 0.0f)
		{
			...
			else if (bIsDefaultStateNotIncludingCrouching || bIsGrabbingThrowable)   // Normal
			{
				...
				{
					bCanChangeDirectionY = CanChangeDirection(AxisValue, YAxisValue);
					if (AxisValue > 0.0f && UKismetMathLibrary::EqualEqual_RotatorRotator(GetActorRotation(), FRotator(0.0f, -90.0f, 0.0f), 1.0f))
					{
						SetActorRotation(FRotator(0.0f, -91.0f, 0.0f));
					}
				}
				AddMovementInput(Direction, AxisValue);
			}
...
void AMainCharacter::CheckEdgeSlip()
{
...
void AMainCharacter::DOF()
{
	if (bIsFP)
	{
		...
		/*PPS.DepthOfFieldFstop = PowerDOF;
		PPS.DepthOfFieldFocalDistance = FocusDistance;
		FirstPersonCameraComponent->PostProcessSettings = PPS;*/
	}
	else
	{
		// Side View DOF Setting
	}
}
...
void AMainCharacter::DefaultWidgetManager()
{
	...
	bCantSwitchView = (!bIsControllable || bIsHangingRope || bIsSliding || bIsOnPole || bIsPickingUp || bIsGrabbingThrowable || bIsAttachedAtMovable || bIsOnLedge || bIsOnLadder || bIsOnPulley || bIsAttachedAtPullBoard || bIsActivatingValve);
	if (bCantSwitchView)
	{
...
void AMainCharacter::CrouchManager()
{
	...
	TArray<AActor*> ToIgnore;
	FHitResult OutHit;
	bool bCanStand = !UKismetSystemLibrary::SphereTraceSingle(GetWorld(), StartLoc, EndLoc, 25.0f, ETraceTypeQuery::TraceTypeQuery1, false,
		ToIgnore, EDrawDebugTrace::None, OutHit, true);
...
void AMainCharacter::ReleaseLMBForMovable()
{
	...
	if (!bIsFP && bIsAttachedAtMovable)
	{
		...
		MovableObj->Box->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Block);
		...
		(
			TriggerCapsuleComponent,
			TriggerCapsuleComponent->GetRelativeLocation(),
...
void AMainCharacter::PressLMBForClimbingPole()
{
	...
	if (bIsPole)
	{
		...
		FRotator HangingRot = /*UKismetMathLibrary::MakeRotFromX(OutHit.ImpactNormal)*/GetActorRotation();
		FLatentActionInfo Info;
		Info.CallbackTarget = this;
		UKismetSystemLibrary::MoveComponentTo(
			TriggerCapsuleComponent,
...
void AMainCharacter::ChangeDirectionManager()
{
	if (bIsFP)
	{
		return;
...
void AMainCharacter::OnMontageEnded(UAnimMontage* AnimMontage, bool bInterrupted)
{
	...
	{
		bIsControllable = true;
		TriggerCapsuleComponent->SetCollisionResponseToChannel(ECollisionChannel::ECC_Vehicle, ECollisionResponse::ECR_Block);
	...
	else if (AnimMontage == M_ChangeDirection)
	{
		bIsChangingDirection = false;
	}
	/*else if (AnimMontage == M_HardLanding)
	{
...
// OnCollisionEnter
void AMainCharacter::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp,
	int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	//if (bIsSprinting/* && UKismetMathLibrary::NearlyEqual_FloatFloat(GetVelocity().Size(), DefaultSprintSpeed, 0.1f) && OtherActor->GetComponentByClass(UStaticMeshComponent::StaticClass())*/)
	//{
	//	if (M_CrashStop)
	//	{
	//		UE_LOG(LogTemp, Log, TEXT("CRASHHH"));
	//		PlayAnimMontage(M_CrashStop);
	//	}
	//}
}
...
{
	//bIsStartingPoint = GameInstance->SaveGameData->MainCharacterStruct.bIsStartingPoint;
	if (bIsTestingMode)
	...
	if (!bIsTestingMode)
	{
		...
		case ECheckPointRespawnType::Crawling:
			if (M_CrawlingCorpse)
			{
				...
				TriggerCapsuleComponent->SetCollisionResponseToChannel(ECollisionChannel::ECC_Vehicle, ECollisionResponse::ECR_Ignore);
	...
	if (!bCantSwitchView)
	{
		if (CameraUtility && bIsDefaultStateNotIncludingCrouching)
		{
			...
			if (!bIsFP)
			{
				...
				//if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Purple, TEXT("-----FP On"));
			...
			else if (bIsFP)
			{
				...
				//if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("-----TP On"));
		...
		{
			// if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 3.0f, FColor::Red, TEXT("CameraUtility is Null"));
		}
	}
}

//??
/**********************************************************Change to bIsControllable**************************************************************************************/
void AMainCharacter::EnableInputCharacter()
{
	EnableInput(PlayerController);
}

void AMainCharacter::DisableInputCharacter()
{
...
void AMainCharacter::SetLadder(AActor* Ladder)
{
	...
	if (OwnLadder)
	{
		//if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("SetLadder()!"));

		bIsLadderReady = true;
	}
	else
	{
		//if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Fail to SetLadder()!"));
...
void AMainCharacter::GrabLadder(bool b)
{
	if (bIsControllable) {
		if (b && OwnLadder && TimeToEnableGrabLadder == 0.0f)
		{
			...
			//if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Orange, FString::Printf(TEXT("dot: %f // deg: %f"), dot, deg));
			...
			if (deg < 45.0f)
			{
				...
				if (OwnLadder) {
					...
					{
						//this->SetActorLocation(LLoc - LFwVec * 15.f + FVector(0, 0, Cast<ALadder>(OwnLadder)->GetGrabHeightOffset() + Offset + Gap * 0.5f + hGap));
						UKismetSystemLibrary::MoveComponentTo
						(
							TriggerCapsuleComponent,
...
void AMainCharacter::ClimbLadderTop()
{
	if (AxisClimbing != 0 && bIsOnLadder && !bIsOnLadderTop)
	{
		...
		//if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Cyan, FString::Printf(TEXT("Climb Top !!")));
	}
}
	...
	if (bIsOnLadder && !bIsOnLadderTop && !bIsLadderClimbing)
	{
		...
		if (bIsLadderSideClimbing)
		{
			if (DeltaLadderSideClimbing < 1.166667/* anim length */)
			{
				...
				//this->SetActorLocation(this->GetActorLocation() + FVector(0, DeltaValue, 0));
			}
			else
			{
