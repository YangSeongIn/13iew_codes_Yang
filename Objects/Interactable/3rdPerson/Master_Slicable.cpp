...
AMaster_Slicable::AMaster_Slicable()
{
	...
	//ProceduralMesh = CreateDefaultSubobject<UProceduralMeshComponent>(TEXT("MasterSlicable_ProceduralMeshComponent"));
...
void AMaster_Slicable::SliceMesh(FVector CharacterForwardVector)
{
	...
	//float DotFValue;
	//FVector::DotProduct(MainCharacter->GetActorForwardVector(), GetActorForwardVector()) > 0 ? DotFValue = -1 : DotFValue = 1;
	...
	//ProceduralMesh->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Ignore);
	//OutHalfMesh->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Ignore);
