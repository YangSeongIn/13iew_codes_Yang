...
void AInteractableObject_Door::Interact()
{
	...
	if (InteractCharacter)
	{
		if (bUseKey)
		{
			if (Key)
			{
				if (InteractCharacter->Inventory.Contains(Key))
				{
					OpenDoor();
				}
				else if(!InteractCharacter->Inventory.Contains(Key))
				{
					CantOpenDoor();
				}
			}
...
void AInteractableObject_Door::RemoveKeyWidget()
{
	if (Key)
	{
		Key->OffKeyInventoryWidget();
	}
}

void AInteractableObject_Door::CantOpenDoor()
{
	if (CantOpenDoorWidgetClass)
	{
		CantOpenDoorWidget = Cast<UCantOpenDoorWidget>(CreateWidget(GetWorld(), CantOpenDoorWidgetClass));
		if (CantOpenDoorWidget)
		{
			CantOpenDoorWidget->AddToViewport();
			FTimerHandle TimerHandle;
			float Timer = 1.0f;
			GetWorld()->GetTimerManager().SetTimer(TimerHandle, FTimerDelegate::CreateLambda([&]()
			{
				if (CantOpenDoorWidget->IsInViewport())
				{
					CantOpenDoorWidget->RemoveFromParent();
				}
...
void AInteractableObject_Door::OpenDoor()
{
